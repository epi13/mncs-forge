...
class MncsDevelopmentService:
    ...
    def _run(self, command: list[str], cwd: Path, *, label: str, timeout: float) -> dict[str, object]:
    ...
    def _read(self, path: Path, *, label: str, byte_cap: int | None = None) -> dict[str, Any]:
        try:
            ...
            raise ForgeError("PROVIDER_CONTRACT_INVALID", f"{label} is not valid JSON: {exc}") from exc
    ...
    def _validate_test_result(value: dict[str, Any]) -> None:
        ...
        if value.get("schema_version") != TEST_RESULT_SCHEMA or value.get("provider") != "mncs-test":
            raise ForgeError("PROVIDER_CONTRACT_INVALID", "test provider did not emit mncs.test-result/1")
        missing = [field for field in required if field not in value]
        if missing or value.get("verdict") not in VERDICTS or not isinstance(value.get("tests"), list):
            raise ForgeError("PROVIDER_CONTRACT_INVALID", f"test result is missing or invalid: {missing}")
        ...
        for index, item in enumerate(value["tests"]):
            if not isinstance(item, dict) or not isinstance(item.get("id"), str) or item.get("verdict") not in VERDICTS:
                raise ForgeError("PROVIDER_CONTRACT_INVALID", f"test result tests[{index}] is not a valid TestExecution projection")
    ...
    def _validate_check(value: dict[str, Any], provider: str) -> None:
        if value.get("schema_version") != CHECK_RESULT_SCHEMA or value.get("provider") != provider:
            raise ForgeError("PROVIDER_CONTRACT_INVALID", f"{provider} did not emit mncs.check-result/1")
    ...
    def _debug_commands(
        ...
        *,
        ...
        common.extend(("--cwd", str(cwd), "--capture", capture_policy, "--max-events", str(max_events), "--timeout", str(timeout)))
        for path in library_paths or []:
        ...
        commands = [
            ("debug-capabilities", capabilities_command, artifacts / "capabilities.json"),
            (
                "debug-import",
                [*prefix, "import-test", str(test_result), "--test-id", test_id, *common, "--output", str(witness)],
                witness,
            ),
            (
                "debug-validation",
                [*prefix, "validate", str(witness), "--output", str(artifacts / "validation.json")],
            ...
            (
                "debug-inspection",
                [*prefix, "inspect", str(witness), "--output", str(artifacts / "inspection.json")],
                artifacts / "inspection.json",
            ),
            (
                "debug-trace",
                [*prefix, "trace", str(witness), "--limit", str(max_events), "--output", str(artifacts / "trace.json")],
            ...
            (
                "debug-replay",
                [*prefix, "replay", str(witness), "--mode", "trace", "--output", str(artifacts / "replay.json")],
        ...
        if minimize:
            ...
            commands.append(("debug-minimization", minimize_command, artifacts / "minimization.json"))
        return commands
    ...
    def _selected_test(self, result: dict[str, Any], test_id: str | None) -> dict[str, Any]:
        ...
        if selected is None:
            raise ForgeError("PROVIDER_CONTRACT_INVALID", "the failing test result has no selectable failing TestExecution")
        if selected.get("verdict") != "FAIL":
            raise ForgeError("PROVIDER_CONTRACT_INVALID", "debug loop selection must reference a failing TestExecution")
        return selected
    ...
    def _source_change(self, path: str, old: str, new: str) -> tuple[Path, str, str]:
        ...
        if not is_within(relative, [*candidate_scopes, *generated_scopes]):
            raise ForgeError("AUTHORITY_FORBIDDEN", "repair path is outside configured candidate/generated scopes")
        authority = self.config.raw.get("authority", {}).get("development", {})
        if not authority.get("may_write_candidates", False) and is_within(relative, candidate_scopes):
            raise ForgeError("AUTHORITY_FORBIDDEN", "development authority cannot write candidate paths")
        if not authority.get("may_write_generated", False) and is_within(relative, generated_scopes):
            raise ForgeError("AUTHORITY_FORBIDDEN", "development authority cannot write generated paths")
        if not isinstance(old, str) or not isinstance(new, str) or not old:
            raise ForgeError("REPAIR_INVALID", "repair requires a non-empty exact source replacement")
        try:
    ...
    def failure_loop(
        ...
        *,
        ...
        if self.config.raw.get("authority", {}).get("development", {}).get("may_run_providers") is not True:
            raise ForgeError("AUTHORITY_FORBIDDEN", "development authority does not permit provider execution")
        if capture_policy not in {"failure-only", "bounded", "events"}:
            raise ForgeError("MNCS_DEBUG_INPUT", "capture_policy must be failure-only, bounded, or events")
        ...
        selected = self._selected_test(test_result, test_id) if test_result["verdict"] == "FAIL" else None
        ...
        if not witness_path.is_file():
            base["verdict"] = "FAIL"
            base["debug"] = {"status": "UNKNOWN", "reason": "mncs-debug did not produce a witness", "execution": debug_executions, "references": []}
            base["diagnosis"] = {"status": "UNKNOWN", "confidence": "none", "statement": "test FAIL is established; debugger output was unavailable"}
            return self._persist(base, output_file)
        ...
        validation = self._read(validation_path, label="mncs-debug validation") if validation_path.is_file() else {}
        if validation.get("schema_version") != VALIDATION_SCHEMA or validation.get("valid") is not True or witness.get("schema_version") != WITNESS_SCHEMA:
            base["verdict"] = "FAIL"
            base["debug"] = {"status": "UNKNOWN", "reason": "mncs-debug witness validation did not establish a valid witness", "execution": debug_executions, "references": [self._ref(witness_path, "mncs-debug-witness", WITNESS_SCHEMA)]}
            base["diagnosis"] = {"status": "UNKNOWN", "confidence": "none", "statement": "test FAIL is established; malformed debug evidence remains INVALID"}
            return self._persist(base, output_file)

        integration = witness.get("integration") if isinstance(witness.get("integration"), dict) else {}
        if integration.get("run_id") != test_result.get("run_id") or integration.get("test_id") != selected_id:
            base["verdict"] = "FAIL"
            base["debug"] = {"status": "UNKNOWN", "reason": "debug witness identity does not match the selected test execution", "execution": debug_executions, "references": [self._ref(witness_path, "mncs-debug-witness", WITNESS_SCHEMA)]}
            base["diagnosis"] = {"status": "UNKNOWN", "confidence": "none", "statement": "test FAIL is established; debug reproduction identity mismatched"}
            return self._persist(base, output_file)
        ...
        for label, path, _ in debug_query_paths:
            ...
            debug_references.append(self._ref(path, f"mncs-debug-{label.removeprefix('debug-')}", expected))
        ...
        if repair_path is None or repair_from is None or repair_to is None:
            raise ForgeError("REPAIR_INVALID", "repair_path, repair_from, and repair_to must be supplied together")
        source_path, before_text, after_text = self._source_change(repair_path, repair_from, repair_to)
        ...
        after_execution = self._run(after_argv, cwd, label="mncs-test-verification", timeout=timeout)
        ...
        base["repair"] = {
            ...
            "replacement_digest": hashlib.sha256(canonical_bytes({"from": repair_from, "to": repair_to})).hexdigest(),
    ...
    def _persist(self, result: dict[str, object], output_file: str | None) -> dict[str, object]:
        ...
        destination = (
            ...
            else self.config.state_dir / "mncs-development" / f"{result['output_identity'].split(':')[-1]}.json"
        )
        destination.parent.mkdir(parents=True, exist_ok=True)
        destination.write_text(pretty_json(result), encoding="utf-8")
        result["artifact"] = {"kind": "forge-mncs-development", "path": str(destination), "sha256": _sha256(destination)}
        return result
